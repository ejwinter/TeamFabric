# fabric-deep-review Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a `/fabric-deep-review` command that audits the TeamFabric framework repo for drift, inconsistencies, technical debt, and token bloat, producing a diff-format proposal report.

**Architecture:** Single command file at `.claude/commands/fabric-deep-review.md` with five discrete named steps, each self-contained. Optional `--focus` flag maps to step names to allow targeted runs. Output goes to `output/deep-review-YYYY-MM-DD.md` (gitignored) and a conversation summary.

**Tech Stack:** Markdown command file (AI instruction format); no code dependencies.
...
| Action | Path | Purpose |
|--------|------|---------|
| Modify | `.gitignore` | Add `output/` so report files are never committed |
| Create | `.claude/commands/fabric-deep-review.md` | The command itself |

---

## Task 1: Add `output/` to root `.gitignore`
...
- [ ] **Step 1: Read the current `.gitignore`**
  ...
  Read `.gitignore` at the TeamFabric root. Current content:
  ```
  .DS_Store
  staging/*
  !staging/README.md
...
- [ ] **Step 2: Add the `output/` entry**

  Append after `.worktrees/`:
  ```
  output/
  ```
  ...
  Final `.gitignore`:
  ```
  # Staging directory: all content except README.md is local/temporary
  .DS_Store
  staging/*
  !staging/README.md
  ...
  # Review output — ephemeral, not committed
  output/
  ```
...
- [ ] **Step 3: Commit**
  ...
  ```bash
  git add .gitignore
  git commit -m "chore: gitignore output/ at repo root for deep-review reports"
  ```
...
**Files:**
- Create: `.claude/commands/fabric-deep-review.md`
...
- [ ] **Step 1: Verify the target directory exists**
  ...
  ls .claude/commands/
  ```
  Expected: `init.md  submit-idea.md  update.md`

- [ ] **Step 2: Create the command file**

  Write `.claude/commands/fabric-deep-review.md` with the following content:
...
- [ ] **Step 3: Verify the file was created**
  ...
  ls .claude/commands/
  ```
  Expected: `fabric-deep-review.md  init.md  submit-idea.md  update.md`

  ```bash
  wc -l .claude/commands/fabric-deep-review.md
  ```
  Expected: ~200 lines
...
- [ ] **Step 4: Commit**
  ...
  ```bash
  git add .claude/commands/fabric-deep-review.md
  git commit -m "feat: add /fabric-deep-review command for periodic framework audit"
  ```
...
- [ ] **Step 1: Run a focused review to verify the command structure works**

  From the TeamFabric root, run:
  ```
  /fabric-deep-review --focus=drift
  ```

  Expected:
  - Command runs Step 1 (Drift) only
  - Steps 2–5 recorded as "Skipped" in the report
  - Report file created at `output/deep-review-YYYY-MM-DD.md`
  - Conversation summary printed with only the Drift section populated

- [ ] **Step 2: Verify `output/` is gitignored**

  ```bash
  git status
  ```
  Expected: `output/` directory does not appear as an untracked file.
...
- [ ] **Step 3: Verify no regressions in existing commands**
  ...
  ls .claude/commands/
  ```
  Expected: `init.md`, `update.md`, `submit-idea.md` all still present alongside the new file.
