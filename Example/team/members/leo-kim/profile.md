# Leo Kim

Role: Junior Engineer
Team Function: Maintenance and Monitoring
Allocation: 100%
Status: Active

## Capacity Adjustments

<!-- Current and upcoming periods of reduced or zero capacity. Cleared by GC. -->
<!-- Omit this section entirely when no adjustments are present. -->

## Summary

Leo handles day-to-day pipeline monitoring and is the primary on-call responder. He is building deeper expertise in pipeline development through maintenance work and mentorship from senior team members.

## Responsibilities

- Primary on-call rotation for pipeline alerts
- Pipeline monitoring and incident triage
- Bug fixes and maintenance tasks
- Learning and growth through guided project work

## Expertise

- Pipeline monitoring and alerting (PagerDuty, Airflow)
- Python scripting
- SQL fundamentals
- Incident triage

## Contact

<!-- Email must match git config user.email for user identification -->
Email: leo.kim@riverdale.org

## Context Log
